  ...
  return (
    <div className="p-8 w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        ...
        <StatsCard title="Exam Done, Not Graduated" value={stats.didExamNotGraduated || 0} />
        <StatsCard title="Finished Program, No Exam" value={stats.finishedNotExam || 0} />
      </div>
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={areaData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                ...
                <XAxis dataKey="name" stroke="#8B4513" tick={{fill: '#8B4513', fontWeight: 600}} />
                <YAxis stroke="#8B4513" tick={{fill: '#8B4513', fontWeight: 600}} />
                <Tooltip 
                  contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.9)', borderRadius: '10px', border: 'none', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}
                  itemStyle={{ fontWeight: 'bold' }}
                />
                <Area type="monotone" dataKey="ICHOOSE" stroke="#8B4513" fillOpacity={1} fill="url(#colorIchoose)" />
                <Area type="monotone" dataKey="ILEAD" stroke="#D2691E" fillOpacity={1} fill="url(#colorIlead)" />
                <Area type="monotone" dataKey="IDO" stroke="#F4A460" fillOpacity={1} fill="url(#colorIdo)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart cx="50%" cy="50%" outerRadius="80%" data={radarData}>
                ...
                <PolarAngleAxis dataKey="subject" tick={{ fill: '#8B4513', fontWeight: 'bold', fontSize: 12 }} />
                <PolarRadiusAxis angle={30} domain={[0, 'dataMax + 2']} tick={false} axisLine={false} />
                <Radar name="Students" dataKey="count" stroke="#8B4513" fill="#8B4513" fillOpacity={0.5} />
                <Tooltip 
                  contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.9)', borderRadius: '10px', border: 'none', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}
