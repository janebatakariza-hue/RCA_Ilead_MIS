...
export default function Statistics() {
  ...
  const filteredStudents = useMemo(() => {
    return students.filter((s) => {
      ...
      const matchStatus =
        statusFilter === "All" || getStatusDisplay(s) === statusFilter;
  ...
  const exportFilteredStudents = () => {
    ...
    const filename =
      `statistics_report_${categoryFilter}_${statusFilter}.pdf`.replace(
        /\s+/g,
        "_",
      );
    doc.save(filename);
    ...
    <div className="p-8 w-full">
      <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-black font-medium mt-2">
            Filter students to see detailed numbers by Year and view exactly who
            they are.
          </p>
        </div>
        <button
      ...
      <div className="glass-panel p-6 mb-8 flex flex-col sm:flex-row gap-6">
        ...
        <div className="flex-1">
          ...
          >
            ...
            <option value="Finished Program, No Exam">
              Finished Program, No Exam
            </option>
            <option value="Exam Done, Not Graduated">
              Exam Done, Not Graduated
            </option>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10">
        {["Year 1", "Year 2", "Year 3"].map((year) => (
          <div
            key={year}
            className="glass-panel p-6 flex flex-col items-center justify-center border-b-4 border-chocolate text-center"
          >
            <h2 className="text-chocolate-dark text-lg font-bold uppercase tracking-widest">
              {year}
            </h2>
            <div className="text-5xl font-black mt-2 text-transparent bg-clip-text bg-gradient-to-br from-chocolate-dark to-chocolate-dark drop-shadow-sm">
              {yearCounts[year]}
            </div>
            <p className="text-chocolate font-semibold text-sm mt-1">
              Matching Students
            </p>
          </div>
        ))}
      ...
      <div>
        <h2 className="text-2xl font-bold text-chocolate-dark mb-4 border-b border-chocolate/20 pb-2 flex justify-between items-center">
          ...
          <span className="text-lg bg-chocolate-dark text-white px-3 py-1 rounded-lg">
