  ...
  return (
    <div className="p-8 w-full">
      <div className="mb-10">
        ...
        <p className="text-chocolate font-medium mt-2">Monitor login history and system activities.</p>
      </div>

      <div className="glass-panel overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-chocolate font-bold">Loading logs...</div>
        ) : (
                ...
                {logs.map((log, index) => (
                  ...
                  >
                    ...
                    <td className="p-4">
                      ...
                      <div className="text-xs text-chocolate font-medium">{log.email}</div>
                    </td>
                    <td className="p-4 font-semibold text-chocolate">
                      <div className="flex items-center gap-1">
                        {log.userRole === 'coordinator' && <ShieldCheck size={14} className="text-green-600" />}
                        <span className={`uppercase text-xs tracking-widest ${log.userRole === 'coordinator' ? 'text-green-700' : 'text-chocolate-dark'}`}>
                          {log.userRole}
                        </span>
                      </div>
                    </td>
                    <td className="p-4 text-chocolate italic font-medium">{log.details}</td>
                    <td className="p-4 text-xs font-mono text-gray-500 text-right">{log.ipAddress || 'Unknown'}</td>
                  </tr>
                ))}
                {logs.length === 0 && (
                  <tr>
                    <td colSpan="6" className="p-12 text-center text-chocolate-dark font-medium text-lg">
